...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Sirius EcoSystem data dir to use
  SIRIUS_HOST=127.0.0.1    Host passed to the onboarded server
  SIRIUS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/sirius-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/sirius-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export SIRIUS_HOST="${SIRIUS_HOST:-127.0.0.1}"
export SIRIUS_PORT="${SIRIUS_PORT:-3100}"
export SIRIUS_OPEN_ON_LISTEN="${SIRIUS_OPEN_ON_LISTEN:-false}"
...
echo "SIRIUS_HOST: $SIRIUS_HOST"
echo "SIRIUS_PORT: $SIRIUS_PORT"
...
env \
  ...
  HOST="$SIRIUS_HOST" \
  PORT="$SIRIUS_PORT" \
  SIRIUS_OPEN_ON_LISTEN="$SIRIUS_OPEN_ON_LISTEN" \
  pnpm siriusly onboard --yes --data-dir "$PC_DATA"
