...
const devServerStatusFilePath = path.join(repoRoot, ".sirius", "dev-server-status.json");
...
const env: NodeJS.ProcessEnv = {
  ...process.env,
  SIRIUS_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.SIRIUS_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.SIRIUS_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.SIRIUS_MIGRATION_PROMPT ??= "never";
  env.SIRIUS_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.SIRIUS_DEPLOYMENT_MODE = "authenticated";
  env.SIRIUS_DEPLOYMENT_EXPOSURE = "private";
  env.SIRIUS_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[sirius] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[sirius] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[sirius] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@siriusly/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[sirius] Command failed with code ${status.code}: pnpm --filter @siriusly/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[sirius] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.SIRIUS_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[sirius] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[sirius] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@siriusly/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[sirius] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@siriusly/server", serverScript, ...forwardedArgs],
