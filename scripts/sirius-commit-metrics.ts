...
const DEFAULT_QUERY = "\"Co-Authored-By: Sirius EcoSystem <noreply@sirius.eco>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/sirius-commit-metrics-cache.json");
...
const SIRIUS_EMAIL = "noreply@sirius.eco";
const SIRIUS_NAME = "sirius";
...
function printHelp() {
  console.log(`Usage: tsx scripts/sirius-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === SIRIUS_EMAIL) || (displayName && displayName.toLowerCase() === SIRIUS_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Sirius EcoSystem commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "sirius-commit-metrics",
