#!/usr/bin/env bash
#
# Kill all local Sirius EcoSystem dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all sirius dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/sirius(-[^/]+)?/' | grep node | grep -v grep || true)

candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.sirius/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.sirius/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.sirius/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/sirius*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.sirius/instances/*/db/postmaster.pid
    "$sibling_root"/.sirius/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No Sirius EcoSystem dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Sirius EcoSystem dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Sirius EcoSystem node processes..."
