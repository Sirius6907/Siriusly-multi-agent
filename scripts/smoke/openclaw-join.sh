...
COMPANY_ID="${COMPANY_ID:-${SIRIUS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-sirius-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-sirius-openclaw-smoke}"
...
Provide one of:
  SIRIUS_AUTH_HEADER=\"Bearer <board-token>\"
  SIRIUS_COOKIE=\"<board-session-cookie>\"
...
log "checking Sirius EcoSystem health"
...
assert_status "200"
if ! grep -q "Sirius EcoSystem OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.sirius.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
