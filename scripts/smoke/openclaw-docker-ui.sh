...
SIRIUS_HOST_PORT="${SIRIUS_HOST_PORT:-3100}"
SIRIUS_HOST_FROM_CONTAINER="${SIRIUS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.sirius-openclaw.override.yml"
...
detect_sirius_base_url() {
  ...
  for candidate in "$SIRIUS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${SIRIUS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${SIRIUS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
sirius_base_url="$(detect_sirius_base_url || true)"
...
Sirius EcoSystem URL for OpenClaw container:
EOF
  if [[ -n "$sirius_base_url" ]]; then
    cat <<EOF
  $sirius_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${SIRIUS_HOST_PORT}
  (Do not use http://127.0.0.1:${SIRIUS_HOST_PORT} inside the container.)
  If Sirius EcoSystem rejects the host, run on host machine:
    pnpm siriusly allowed-hostname host.docker.internal
  Then restart Sirius EcoSystem and re-run this script.
EOF
  fi
  cat <<EOF
