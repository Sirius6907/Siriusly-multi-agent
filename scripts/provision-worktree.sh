...
base_cwd="${SIRIUS_WORKSPACE_BASE_CWD:?SIRIUS_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${SIRIUS_WORKSPACE_CWD:?SIRIUS_WORKSPACE_CWD is required}"
sirius_home="${SIRIUS_HOME:-$HOME/.sirius}"
sirius_instance_id="${SIRIUS_INSTANCE_ID:-default}"
sirius_dir="$worktree_cwd/.sirius"
worktree_config_path="$sirius_dir/config.json"
worktree_env_path="$sirius_dir/.env"
worktree_name="${SIRIUS_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${SIRIUS_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.sirius/config.json" || -L "$base_cwd/.sirius/config.json" ) ]]; then
  source_config_path="$base_cwd/.sirius/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$sirius_home/instances/$sirius_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm siriusly --help >/dev/null 2>&1; then
    pnpm siriusly worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v siriusly >/dev/null 2>&1; then
    siriusly worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  SIRIUS_WORKTREES_DIR="${SIRIUS_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const siriusDir = process.env.SIRIUS_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.SIRIUS_WORKTREES_DIR) ?? "~/.sirius-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(siriusDir, "config.json");
  const envPath = path.resolve(siriusDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "sirius",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.SIRIUS_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.SIRIUS_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.SIRIUS_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "SIRIUS_HOME=" + JSON.stringify(worktreeHome),
    "SIRIUS_INSTANCE_ID=" + JSON.stringify(instanceId),
    "SIRIUS_CONFIG=" + JSON.stringify(configPath),
    "SIRIUS_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "SIRIUS_IN_WORKTREE=true",
    "SIRIUS_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.SIRIUS_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("SIRIUS_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "siriusly CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
