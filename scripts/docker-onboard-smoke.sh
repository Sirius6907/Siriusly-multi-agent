...
IMAGE_NAME="${IMAGE_NAME:-sirius-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
SIRIUS_VERSION="${SIRIUS_VERSION:-latest}"
...
SIRIUS_DEPLOYMENT_MODE="${SIRIUS_DEPLOYMENT_MODE:-authenticated}"
SIRIUS_DEPLOYMENT_EXPOSURE="${SIRIUS_DEPLOYMENT_EXPOSURE:-private}"
SIRIUS_PUBLIC_URL="${SIRIUS_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@sirius.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-sirius-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e SIRIUS_DEPLOYMENT_MODE="$SIRIUS_DEPLOYMENT_MODE" \
      -e SIRIUS_DEPLOYMENT_EXPOSURE="$SIRIUS_DEPLOYMENT_EXPOSURE" \
      -e SIRIUS_PUBLIC_URL="$SIRIUS_PUBLIC_URL" \
      -e SIRIUS_HOME="/sirius" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "siriusly@${SIRIUS_VERSION}" auth bootstrap-ceo --data-dir "$SIRIUS_HOME" --base-url "$SIRIUS_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$SIRIUS_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$SIRIUS_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$SIRIUS_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$SIRIUS_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$SIRIUS_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$SIRIUS_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $SIRIUS_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $SIRIUS_DEPLOYMENT_MODE/$SIRIUS_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sirius-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$SIRIUS_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $SIRIUS_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$SIRIUS_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $SIRIUS_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
