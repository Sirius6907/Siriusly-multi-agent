---
name: sirius-create-plugin
description: >
  Create new Sirius EcoSystem plugins with the current alpha SDK/runtime. Use when
...
# Create a Sirius EcoSystem Plugin

Use this skill when the task is to create, scaffold, or document a Sirius EcoSystem plugin.
...
node packages/plugins/create-sirius-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Sirius EcoSystem repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.sirius-sdk/`:

```bash
pnpm --filter @siriusly/create-sirius-plugin build
node packages/plugins/create-sirius-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Sirius EcoSystem from an absolute local path during development
