...
import type { AdapterModel } from "@siriusly/adapter-utils";
import { asString, runChildProcess } from "@siriusly/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.SIRIUS_PI_COMMAND === "string" &&
    process.env.SIRIUS_PI_COMMAND.trim().length > 0
      ? process.env.SIRIUS_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["SIRIUS_", "npm_", "NPM_"] as const;
