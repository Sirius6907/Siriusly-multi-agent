...
Use when:
- You want Sirius EcoSystem to run Pi (the AI coding agent) locally as the agent runtime
...
- Sirius EcoSystem requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/siriuss/ and resumed with --session.
