...
Use when:
- You want Sirius EcoSystem to run Cursor Agent CLI locally as the agent runtime
...
- Sirius EcoSystem auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$sirius" and related skills on local runs.
- Sirius EcoSystem auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
