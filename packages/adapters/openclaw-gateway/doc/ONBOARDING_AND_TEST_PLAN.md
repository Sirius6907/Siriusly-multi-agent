...
## Scope
This plan is now **gateway-only**. Sirius EcoSystem supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Sirius EcoSystem skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Sirius EcoSystem task.
...
```bash
pnpm --filter @siriusly/server test:run -- openclaw-gateway
pnpm --filter @siriusly/server typecheck
pnpm --filter @siriusly/ui typecheck
pnpm --filter siriusly typecheck
```
