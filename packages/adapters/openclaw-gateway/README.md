...
This document describes how `@siriusly/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Sirius EcoSystem logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Sirius EcoSystem `runId`)
