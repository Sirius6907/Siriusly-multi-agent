...
import { asString, parseObject } from "@siriusly/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Sirius EcoSystem server host.",
        });
      }
    } catch (err) {
      checks.push({
