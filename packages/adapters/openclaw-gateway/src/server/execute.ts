...
} from "@siriusly/adapter-utils";
import { asNumber, asString, buildSiriusEnv, parseObject } from "@siriusly/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "sirius";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "sirius";
  if (input.strategy === "run") return `sirius:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `sirius:issue:${input.issueId}`;
  return fallback;
}
...
function resolveSiriusApiUrlOverride(value: unknown): string | null {
...
function buildSiriusEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const siriusApiUrlOverride = resolveSiriusApiUrlOverride(ctx.config.siriusApiUrl);
  const siriusEnv: Record<string, string> = {
    ...buildSiriusEnv(ctx.agent),
    SIRIUS_RUN_ID: ctx.runId,
  };

  if (siriusApiUrlOverride) {
    siriusEnv.SIRIUS_API_URL = siriusApiUrlOverride;
  }
  if (wakePayload.taskId) siriusEnv.SIRIUS_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) siriusEnv.SIRIUS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) siriusEnv.SIRIUS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) siriusEnv.SIRIUS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) siriusEnv.SIRIUS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    siriusEnv.SIRIUS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return siriusEnv;
}

function buildWakeText(payload: WakePayload, siriusEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/sirius-claimed-api-key.json";
  const orderedKeys = [
    "SIRIUS_RUN_ID",
    "SIRIUS_AGENT_ID",
    "SIRIUS_COMPANY_ID",
    ...
    "SIRIUS_TASK_ID",
    "SIRIUS_WAKE_REASON",
    "SIRIUS_WAKE_COMMENT_ID",
    "SIRIUS_APPROVAL_ID",
    "SIRIUS_APPROVAL_STATUS",
    "SIRIUS_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = siriusEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = siriusEnv.SIRIUS_API_URL ?? "<set SIRIUS_API_URL>";

  const lines = [
    "Sirius EcoSystem wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `SIRIUS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load SIRIUS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $SIRIUS_API_KEY on every API call.",
    "- Use X-Sirius EcoSystem-Run-Id: $SIRIUS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: SIRIUS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$SIRIUS_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$SIRIUS_COMPANY_ID/issues?assigneeAgentId=$SIRIUS_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardSiriusPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  siriusEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateSirius = parseObject(payloadTemplate.sirius);
  const workspace = asRecord(ctx.context.siriusWorkspace);
  const workspaces = Array.isArray(ctx.context.siriusWorkspaces)
    ? ctx.context.siriusWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.siriusRuntimeServiceIntents)
    ? ctx.context.siriusRuntimeServiceIntents.filter(
  ...
  const standardSiriusEcoSystem: Record<string, unknown> = {
    ...
    apiUrl: siriusEnv.SIRIUS_API_URL ?? null,
  };
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "sirius-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const siriusEnv = buildSiriusEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, siriusEnv);
  ...
  const siriusPayload = buildStandardSiriusPayload(ctx, wakePayload, siriusEnv, payloadTemplate);
