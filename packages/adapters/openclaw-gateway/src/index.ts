...
Use when:
- You want Sirius EcoSystem to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Sirius EcoSystem server.
...
- siriusApiUrl (string, optional): absolute Sirius EcoSystem base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default sirius)

Standard outbound payload additions:
- sirius (object): standardized SiriusEcoSystem context added to every gateway agent request
- sirius.workspace (object, optional): resolved execution workspace for this run
- sirius.workspaces (array, optional): additional workspace hints SiriusEcoSystem exposed to the run
- sirius.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
