...
const SIRIUS_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface SiriusSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: SiriusSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "sirius_required",
      originLabel: "Required by Sirius EcoSystem",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Sirius EcoSystem",
    readOnly: false,
  };
}
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "SIRIUS_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildSiriusEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    SIRIUS_AGENT_ID: agent.id,
    SIRIUS_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.SIRIUS_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.SIRIUS_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.SIRIUS_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.SIRIUS_API_URL = apiUrl;
  return vars;
}
...
export async function resolveSiriusSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...SIRIUS_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listSiriusSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<SiriusSkillEntry[]> {
  const root = await resolveSiriusSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `siriusly/sirius/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Sirius EcoSystem skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Sirius EcoSystem cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredSiriusRuntimeSkills(value: unknown): SiriusSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: SiriusSkillEntry[] = [];
...
export async function readSiriusRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<SiriusSkillEntry[]> {
  const configuredEntries = normalizeConfiguredSiriusRuntimeSkills(config.siriusRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listSiriusSkillEntries(moduleDir, additionalCandidates);
}

export async function readSiriusSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listSiriusSkillEntries(moduleDir);
...
export function readSiriusSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.siriusSkillSync;
...
function canonicalizeDesiredSiriusSkillReference(
  reference: string,
...
export function resolveSiriusDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readSiriusSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredSiriusSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeSiriusSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.siriusSkillSync;
  const current =
  ...
  next.siriusSkillSync = current;
  return next;
}

export async function ensureSiriusSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Sirius EcoSystem server itself is started from
    // within a Claude Code session (e.g. `npx siriusly run` in a terminal
