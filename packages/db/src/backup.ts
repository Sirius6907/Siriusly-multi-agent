...
function resolveSiriusInstanceId(): string {
  const raw = process.env.SIRIUS_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveSiriusHomeDir(), "instances", resolveSiriusInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveSiriusHomeDir(), "instances", resolveSiriusInstanceId(), "data", "backups");
}
