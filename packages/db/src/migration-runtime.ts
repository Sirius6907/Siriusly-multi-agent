...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://sirius:sirius@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "sirius");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://sirius:sirius@127.0.0.1:${preferredPort}/sirius`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://sirius:sirius@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "sirius");
    return {
      connectionString: `postgres://sirius:sirius@127.0.0.1:${port}/sirius`,
  ...
  const adminConnectionString = `postgres://sirius:sirius@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "sirius");

  return {
    connectionString: `postgres://sirius:sirius@127.0.0.1:${selectedPort}/sirius`,
