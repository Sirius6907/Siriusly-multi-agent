...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".sirius-sdk");
...
/**
 * Generate a complete Sirius EcoSystem plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Sirius EcoSystem plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "sirius-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["sirius", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@siriusly/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@siriusly/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { SiriusPluginManifestV1 } from "@siriusly/plugin-sdk";

const manifest: SiriusPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@siriusly/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@siriusly/plugin-sdk\` and \`@siriusly/shared\` from a local Sirius EcoSystem checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.sirius-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-sirius-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <sirius-sdk-path>]");
    process.exit(1);
  }
