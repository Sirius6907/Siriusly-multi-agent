...
type SiriusSprite = (typeof SIRIUS_SPRITES)[number];
...
function spriteSize(sprite: SiriusSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = SIRIUS_SPRITES[(baseCol + baseRow) % SIRIUS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = SIRIUS_SPRITES[Math.floor(Math.random() * SIRIUS_SPRITES.length)]!;
