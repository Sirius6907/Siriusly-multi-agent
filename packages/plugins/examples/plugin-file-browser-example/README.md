...
Example Sirius EcoSystem plugin that demonstrates:
...
```bash
pnpm --filter @siriusly/plugin-file-browser-example build
pnpm siriusly plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm siriusly plugin uninstall sirius-file-browser-example --force
```
...
- Optional: use `sirius-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
