...
Official TypeScript SDK for Sirius EcoSystem plugin authors.

- **Worker SDK:** `@siriusly/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@siriusly/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@siriusly/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@siriusly/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@siriusly/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@siriusly/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@siriusly/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@siriusly/plugin-sdk/ui/hooks` | Hooks only |
| `@siriusly/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@siriusly/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@siriusly/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@siriusly/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@siriusly/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@siriusly/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Sirius EcoSystem app. They can call ordinary Sirius EcoSystem HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Sirius EcoSystem instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@siriusly/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@siriusly/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Sirius EcoSystem information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@siriusly/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@siriusly/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@siriusly/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@siriusly/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@siriusly/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@siriusly/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@siriusly/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@siriusly/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@siriusly/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@siriusly/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@siriusly/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@siriusly/plugin-sdk/bundlers";
...
```bash
sirius-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@siriusly/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__sirius__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__sirius__/events` streams `reload` SSE events on UI build changes
