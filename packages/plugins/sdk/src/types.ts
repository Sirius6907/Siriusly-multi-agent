 ...
 * from `@siriusly/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @siriusly/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Sirius EcoSystem object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Sirius EcoSystem UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Sirius EcoSystem domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Sirius EcoSystem domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Sirius EcoSystem secret provider
 ...
 * ```
 *
 * @see usePluginStream in `@siriusly/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@siriusly/plugin-sdk";
 *
