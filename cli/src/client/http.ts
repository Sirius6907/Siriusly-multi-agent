...
export class SiriusApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-sirius-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Sirius EcoSystem API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Sirius EcoSystem server is not running, the configured URL is wrong, or the request is being blocked before it reaches SiriusEcoSystem.",
    "",
    "Try:",
    "- Start Sirius EcoSystem with `pnpm dev` or `pnpm siriusly run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Sirius EcoSystem is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`SIRIUS_API_URL\`.`,
  );
  return lines.join("\n");
}
