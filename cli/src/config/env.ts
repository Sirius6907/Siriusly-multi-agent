...
const JWT_SECRET_ENV_KEY = "SIRIUS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Sirius EcoSystem CLI commands",
...
export function resolveSiriusEnvFile(configPath?: string): string {
...
export function loadSiriusEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeSiriusEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readSiriusEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeSiriusEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeSiriusEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readSiriusEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeSiriusEnvEntries(next, filePath);
  return next;
}
