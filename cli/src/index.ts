...
const DATA_DIR_OPTION_HELP =
  "Sirius EcoSystem data directory root (isolates state from ~/.sirius)";

program
  .name("siriusly")
  .description("Sirius EcoSystem CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadSiriusEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start Sirius EcoSystem immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Sirius EcoSystem setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "sirius")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Sirius EcoSystem")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Sirius EcoSystem server API")
