...
export const DEFAULT_WORKTREE_HOME = "~/.sirius-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".sirius");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: SiriusConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "sirius",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    SIRIUS_HOME: paths.homeDir,
    SIRIUS_INSTANCE_ID: paths.instanceId,
    SIRIUS_CONFIG: paths.configPath,
    SIRIUS_CONTEXT: paths.contextPath,
    SIRIUS_IN_WORKTREE: "true",
    ...(branding?.name ? { SIRIUS_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { SIRIUS_WORKTREE_COLOR: branding.color } : {}),
  };
}
