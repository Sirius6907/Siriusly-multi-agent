...
type OnboardDefaults = Pick<SiriusConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "SIRIUS_PUBLIC_URL",
  "DATABASE_URL",
  "SIRIUS_DB_BACKUP_ENABLED",
  "SIRIUS_DB_BACKUP_INTERVAL_MINUTES",
  "SIRIUS_DB_BACKUP_RETENTION_DAYS",
  "SIRIUS_DB_BACKUP_DIR",
  "SIRIUS_DEPLOYMENT_MODE",
  "SIRIUS_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "SIRIUS_ALLOWED_HOSTNAMES",
  "SIRIUS_AUTH_BASE_URL_MODE",
  "SIRIUS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "SIRIUS_STORAGE_PROVIDER",
  "SIRIUS_STORAGE_LOCAL_DIR",
  "SIRIUS_STORAGE_S3_BUCKET",
  "SIRIUS_STORAGE_S3_REGION",
  "SIRIUS_STORAGE_S3_ENDPOINT",
  "SIRIUS_STORAGE_S3_PREFIX",
  "SIRIUS_STORAGE_S3_FORCE_PATH_STYLE",
  "SIRIUS_SECRETS_PROVIDER",
  "SIRIUS_SECRETS_STRICT_MODE",
  "SIRIUS_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveSiriusInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.SIRIUS_PUBLIC_URL?.trim() ||
    process.env.SIRIUS_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.SIRIUS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.SIRIUS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.SIRIUS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.SIRIUS_ALLOWED_HOSTNAMES
    ? process.env.SIRIUS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.SIRIUS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.SIRIUS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.SIRIUS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.SIRIUS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.SIRIUS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.SIRIUS_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.SIRIUS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.SIRIUS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.SIRIUS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.SIRIUS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.SIRIUS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.SIRIUS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.SIRIUS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.SIRIUS_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.SIRIUS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "SIRIUS_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<SiriusConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printSiriusCliBanner();
  p.intro(pc.bgCyan(pc.black(" siriusly onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveSiriusInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: SiriusConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Sirius EcoSystem install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("siriusly configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("SIRIUS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.SIRIUS_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("SIRIUS_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("SIRIUS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: SIRIUS_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("siriusly run")}`,
        `Reconfigure later: ${pc.cyan("siriusly configure")}`,
        `Diagnose setup: ${pc.cyan("siriusly doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (!shouldRunNow && !opts.invokedByRun && process.stdin.isTTY && process.stdout.isTTY) {
      const answer = await p.confirm({
        message: "Start Sirius EcoSystem now?",
        initialValue: true,
      });
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.SIRIUS_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Sirius EcoSystem setup is ready.");
    return;
  }
  ...
  let llm: SiriusConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@siriusly/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `siriusly doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("siriusly run")}`,
        `Then: ${pc.cyan("siriusly auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
