...
import type { Agent } from "@siriusly/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveSiriusSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export SIRIUS_API_URL='${escaped(input.apiBase)}'`,
    `export SIRIUS_COMPANY_ID='${escaped(input.companyId)}'`,
    `export SIRIUS_AGENT_ID='${escaped(input.agentId)}'`,
    `export SIRIUS_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Sirius EcoSystem skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Sirius EcoSystem skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveSiriusSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Sirius EcoSystem skills directory. Expected ./skills in the repo checkout.",
              );
            }
