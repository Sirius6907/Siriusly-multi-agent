...
import { ApiRequestError, SiriusApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Sirius EcoSystem config file")
    .option("-d, --data-dir <path>", "Sirius EcoSystem data directory root (isolates state from ~/.sirius)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Sirius EcoSystem API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.SIRIUS_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.SIRIUS_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.SIRIUS_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set SIRIUS_COMPANY_ID, or set context profile companyId via `siriusly context set`.",
    );
  }

  const api = new SiriusApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.SIRIUS_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.SIRIUS_SERVER_PORT || "");
