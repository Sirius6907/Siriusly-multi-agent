...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@siriusly/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://sirius:sirius@127.0.0.1:${port}/sirius`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" sirius db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveSiriusInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "sirius";
