...
import { ensureAgentJwtSecret, loadSiriusEnvFile, mergeSiriusEnvEntries, readSiriusEnvEntries, resolveSiriusEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { SiriusConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printSiriusCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.SIRIUS_CONFIG;
...
const WORKTREE_NAME_PREFIX = "sirius-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.SIRIUS_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.SIRIUS_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromSiriusConfig(config: SiriusConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromSiriusConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.sirius${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".sirius", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".sirius", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.sirius"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: SiriusConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://sirius:sirius@127.0.0.1:${port}/sirius`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: SiriusConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.SIRIUS_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.SIRIUS_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.SIRIUS_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.SIRIUS_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: SiriusConfig;
  targetConfig: SiriusConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveSiriusEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readSiriusEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://sirius:sirius@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "sirius");
    const targetConnectionString = `postgres://sirius:sirius@127.0.0.1:${targetHandle.port}/sirius`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readSiriusEnvEntries(resolveSiriusEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.SIRIUS_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.SIRIUS_AGENT_JWT_SECRET);
  mergeSiriusEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { SIRIUS_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadSiriusEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Sirius EcoSystem inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printSiriusCliBanner();
  p.intro(pc.bgCyan(pc.black(" siriusly worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printSiriusCliBanner();
  p.intro(pc.bgCyan(pc.black(" siriusly worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasSiriusConfig: existsSync(path.resolve(worktreePath, ".sirius", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printSiriusCliBanner();
  p.intro(pc.bgCyan(pc.black(" siriusly worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveSiriusEnvFile(configPath);
  const envEntries = readSiriusEnvEntries(envPath);
  const out = {
    SIRIUS_CONFIG: configPath,
    ...(envEntries.SIRIUS_HOME ? { SIRIUS_HOME: envEntries.SIRIUS_HOME } : {}),
    ...(envEntries.SIRIUS_INSTANCE_ID ? { SIRIUS_INSTANCE_ID: envEntries.SIRIUS_INSTANCE_ID } : {}),
    ...(envEntries.SIRIUS_CONTEXT ? { SIRIUS_CONTEXT: envEntries.SIRIUS_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasSiriusConfig)
      .map((choice) => path.resolve(choice.worktree, ".sirius", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readSiriusEnvEntries(resolveSiriusEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Sirius EcoSystem once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasSiriusConfig ? "sirius" : "no-sirius-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".sirius", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".sirius", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .sirius/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasSiriusConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Sirius EcoSystem worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasSiriusConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Sirius EcoSystem worktrees were found. Run `siriusly worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Sirius EcoSystem configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Sirius EcoSystem instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Sirius EcoSystem instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with sirius- if needed (created at ~/sirius-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: SIRIUS_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: SIRIUS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source SIRIUS_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Sirius EcoSystem instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Sirius EcoSystem worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with sirius- if needed")
