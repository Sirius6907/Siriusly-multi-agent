...
import { ApiConnectionError, ApiRequestError, SiriusApiClient } from "../client/http.js";

describe("SiriusApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new SiriusApiClient({
    ...
    expect(headers["x-sirius-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new SiriusApiClient({ apiBase: "http://localhost:3100" });
