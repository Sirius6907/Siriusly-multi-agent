/**
 * esbuild configuration for building the siriusly CLI for npm.
 *
 * Bundles all workspace packages (@siriusly/*) into a single file.
...
// to npm and resolved at runtime (e.g. @siriusly/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@siriusly/")) {
      externals.add(name);
    }
  }
