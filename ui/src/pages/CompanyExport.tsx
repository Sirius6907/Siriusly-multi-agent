...
/**
 * Filter .sirius.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterSiriusYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://sirius.eco) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm siriusly company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Sirius EcoSystem](https://sirius.eco) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Sirius EcoSystem](https://sirius.eco) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .sirius.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .sirius.yaml
    const yamlPath = exportData.siriusExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterSiriusYaml(exportData.files[yamlPath], checkedFiles);
    }
