...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("sirius-worktree-enabled") !== "true") return null;

  const name = readMetaContent("sirius-worktree-name");
  const color = normalizeHexColor(readMetaContent("sirius-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("sirius-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
