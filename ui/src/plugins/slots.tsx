 ...
 *
 * Plugin bundles are built with `external: ["@siriusly/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__siriusPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__siriusPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__siriusPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__siriusPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@siriusly/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@siriusly/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@siriusly/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@siriusly/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@siriusly/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__siriusPluginBridge__) {
 ...
 * self-register via `window.siriusPlugins.registerReactComponent()`.
 ...
 * of `@siriusly/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
