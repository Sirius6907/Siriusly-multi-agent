...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@siriusly/db";
import type { DeploymentMode } from "@siriusly/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  siriusUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).siriusUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.siriusUpgradeContext = context;
