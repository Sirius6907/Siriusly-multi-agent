...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "siriusly/sirius/sirius-create-agent";
  ...
  it("defaults to mounting all built-in Sirius EcoSystem skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === siriusKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat Sirius EcoSystem skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("sirius-claude-user-skills-");
