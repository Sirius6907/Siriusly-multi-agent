...
import { resolveSiriusConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@siriusly/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "sirius@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Sirius EcoSystem Test"]);
...
describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Sirius EcoSystem and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/sirius",
      SIRIUS_HOME: "/tmp/sirius-home",
      SIRIUS_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.SIRIUS_HOME).toBeUndefined();
    expect(sanitized.SIRIUS_INSTANCE_ID).toBeUndefined();
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".sirius", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$SIRIUS_WORKSPACE_BRANCH\" > .sirius-provision-branch",
        "printf '%s\\n' \"$SIRIUS_WORKSPACE_BASE_CWD\" > .sirius-provision-base",
        "printf '%s\\n' \"$SIRIUS_WORKSPACE_CREATED\" > .sirius-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".sirius-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".sirius-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".sirius-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".sirius-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local Sirius EcoSystem config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const siriusHome = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(siriusHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = instanceId;
    process.env.SIRIUS_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/sirius"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".sirius", "config.json");
      const envPath = path.join(workspace.cwd, ".sirius", ".env");
      ...
      expect(envContents).toContain(`SIRIUS_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
      expect(envContents).toContain(`SIRIUS_INSTANCE_ID=${JSON.stringify(expectedInstanceId)}`);
      expect(envContents).toContain(`SIRIUS_CONFIG=${JSON.stringify(configPath)}`);
      expect(envContents).toContain("SIRIUS_IN_WORKTREE=true");
      expect(envContents).toContain(
        `SIRIUS_WORKTREE_NAME=${JSON.stringify("PAP-885-show-worktree-banner")}`,
      );

      process.chdir(workspace.cwd);
      expect(resolveSiriusConfigPath()).toBe(configPath);
    } finally {
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-workspace-"));
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".sirius", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.SIRIUS_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              SIRIUS_HOME: "{{workspace.cwd}}/.sirius/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".sirius", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".sirius", "runtime-services"));
  });

  it("does not leak parent Sirius EcoSystem instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "siriusConfig: process.env.SIRIUS_CONFIG ?? null,",
          "siriusHome: process.env.SIRIUS_HOME ?? null,",
          "siriusInstanceId: process.env.SIRIUS_INSTANCE_ID ?? null,",
    ...
    process.env.SIRIUS_CONFIG = "/tmp/base-sirius-config.json";
    process.env.SIRIUS_HOME = "/tmp/base-sirius-home";
    process.env.SIRIUS_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/sirius";
    ...
    expect(captured.siriusConfig).toBeNull();
    expect(captured.siriusHome).toBeNull();
    expect(captured.siriusInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-sibling-"));
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("sirius-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-reconcile-"));
    const siriusHome = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-home-"));
    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
