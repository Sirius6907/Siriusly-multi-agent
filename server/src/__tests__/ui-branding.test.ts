...
<head>
    <!-- SIRIUS_RUNTIME_BRANDING_START -->
    <!-- SIRIUS_RUNTIME_BRANDING_END -->
    <!-- SIRIUS_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from SIRIUS_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ SIRIUS_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SIRIUS_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SIRIUS_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      SIRIUS_IN_WORKTREE: "true",
      SIRIUS_WORKTREE_NAME: "sirius-pr-432",
      SIRIUS_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("sirius-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="sirius-worktree-name"');
    expect(meta).toContain('content="sirius-pr-432"');
    expect(meta).toContain('name="sirius-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="sirius-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="sirius-worktree-name"');
  });
});
