...
import type { CompanyPortabilityFileEntry } from "@siriusly/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "sirius",
        name: "sirius",
        description: "Sirius EcoSystem coordination skill",
        markdown: "---\nname: sirius\ndescription: Sirius EcoSystem coordination skill\n---\n\n# SiriusEcoSystem\n",
        sourceType: "github",
        sourceLocator: "https://github.com/siriusly/sirius/tree/master/skills/sirius",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/siriusly/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "siriusly",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/siriusly/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "siriusly",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Sirius EcoSystem extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Sirius EcoSystem"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${siriusKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/siriusly/sirius/sirius/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/siriusly/sirius/sirius/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/siriusly/sirius/sirius/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".sirius.yaml"]);
    expect(extension).toContain('schema: "sirius/v1"');
    ...
    expect(extension).not.toContain("siriusSkillSync");
  ...
  it("exports default sidebar order into the Sirius EcoSystem extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".sirius.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/siriusly/sirius/sirius/SKILL.md"])).toContain("# Sirius EcoSystem");
    expect(asTextFile(exported.files["skills/siriusly/sirius/sirius/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/siriusly/sirius/sirius/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/siriusly/sirius/sirius/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/siriusly/sirius/sirius/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .sirius.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".sirius.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-sirius",
        companyId: "company-1",
        key: "siriusly/sirius/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/siriusly/sirius/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "sirius_bundled",
          owner: "siriusly",
          repo: "sirius",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/siriusly/sirius/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/siriusly/sirius/release-changelog/SKILL.md"])).toContain("siriusly/sirius/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/sirius",
            repoUrl: "https://github.com/siriusly/sirius.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .sirius-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/siriusly/sirius.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/sirius");
    ...
    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Sirius EcoSystem",
    });
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Sirius EcoSystem",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/siriusly/sirius.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".sirius.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/siriusly/sirius.git"');
    expect(extension).toContain('projectWorkspaceKey: "sirius"');
  ...
  it("reads env inputs back from .sirius.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Sirius EcoSystem routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "sirius-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Sirius EcoSystem" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Sirius EcoSystem"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .sirius.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Sirius EcoSystem");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".sirius.yaml"] = `${exported.files[".sirius.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
