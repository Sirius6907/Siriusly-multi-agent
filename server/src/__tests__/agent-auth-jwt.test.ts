...
describe("agent local JWT", () => {
  const secretEnv = "SIRIUS_AGENT_JWT_SECRET";
  const ttlEnv = "SIRIUS_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "SIRIUS_AGENT_JWT_ISSUER";
  const audienceEnv = "SIRIUS_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "sirius";
    process.env[audienceEnv] = "sirius-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
