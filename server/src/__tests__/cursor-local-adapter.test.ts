...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@siriusly/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@siriusly/adapter-cursor-local/ui";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$SIRIUS_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $SIRIUS_API_KEY\"";
