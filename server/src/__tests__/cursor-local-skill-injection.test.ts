...
import { ensureCursorSkillsInjected } from "@siriusly/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Sirius EcoSystem skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("sirius-cursor-skills-src-");
    const skillsHome = await makeTempDir("sirius-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "sirius");
    await createSkillDir(skillsDir, "sirius-create-agent");
    ...
    const injectedA = path.join(skillsHome, "sirius");
    const injectedB = path.join(skillsHome, "sirius-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "sirius")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "sirius-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "sirius"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "sirius-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("sirius-cursor-preserve-src-");
    const skillsHome = await makeTempDir("sirius-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "sirius");
    ...
    expect((await fs.lstat(path.join(skillsHome, "sirius-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("sirius-cursor-fail-src-");
    const skillsHome = await makeTempDir("sirius-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
