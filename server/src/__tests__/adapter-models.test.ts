...
import { models as codexFallbackModels } from "@siriusly/adapter-codex-local";
import { models as cursorFallbackModels } from "@siriusly/adapter-cursor-local";
import { models as opencodeFallbackModels } from "@siriusly/adapter-opencode-local";
import { resetOpenCodeModelsCacheForTests } from "@siriusly/adapter-opencode-local/server";
...
describe("adapter model listing", () => {
  beforeEach(() => {
    ...
    delete process.env.SIRIUS_OPENCODE_COMMAND;
  ...
  it("returns opencode fallback models including gpt-5.4", async () => {
    process.env.SIRIUS_OPENCODE_COMMAND = "__sirius_missing_opencode_command__";
