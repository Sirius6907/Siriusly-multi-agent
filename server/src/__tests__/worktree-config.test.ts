...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const siriusDir = path.join(worktreeRoot, ".sirius");
    const configPath = path.join(siriusDir, "config.json");
    const envPath = path.join(siriusDir, ".env");
    const sharedRoot = path.join(tempRoot, ".sirius", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".sirius-worktrees");

    await fs.mkdir(siriusDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Sirius EcoSystem environment variables",
        "SIRIUS_IN_WORKTREE=true",
        "SIRIUS_WORKTREE_NAME=PAP-884-ai-commits-component",
        "SIRIUS_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.SIRIUS_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.SIRIUS_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`SIRIUS_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('SIRIUS_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`SIRIUS_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`SIRIUS_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('SIRIUS_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.SIRIUS_HOME).toBe(isolatedHome);
    expect(process.env.SIRIUS_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Sirius EcoSystem environment variables",
        "SIRIUS_IN_WORKTREE=true",
        "SIRIUS_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.SIRIUS_IN_WORKTREE = "true";
    process.env.SIRIUS_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.SIRIUS_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".sirius-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".sirius", "worktrees");
    ...
    const siriusDir = path.join(currentWorktreeRoot, ".sirius");
    const configPath = path.join(siriusDir, "config.json");
    const envPath = path.join(siriusDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".sirius", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.SIRIUS_IN_WORKTREE = "true";
    process.env.SIRIUS_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.SIRIUS_HOME = isolatedHome;
    process.env.SIRIUS_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.SIRIUS_CONFIG = configPath;
