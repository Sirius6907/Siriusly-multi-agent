...
import type { agents } from "@siriusly/db";
import { sessionCodec as codexSessionCodec } from "@siriusly/adapter-codex-local/server";
  ...
  it("reuses the persisted execution workspace path instead of deriving a new worktree", () => {
    const result = buildRealizedExecutionWorkspaceFromPersisted({
      ...
      workspace: {
        ...
        repoUrl: "https://example.com/sirius.git",
        baseRef: "main",
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[sirius] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables Sirius EcoSystem-managed rotation by default for codex and claude local", () => {
