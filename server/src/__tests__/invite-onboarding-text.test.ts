...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Sirius EcoSystem OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Sirius EcoSystem base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("siriusApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.siriusApiUrl");
    expect(text).toContain("~/.openclaw/workspace/sirius-claimed-api-key.json");
    expect(text).toContain("SIRIUS_API_KEY");
