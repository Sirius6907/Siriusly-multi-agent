...
describe("gemini local skill sync", () => {
  ...
  it("reports configured Sirius EcoSystem skills and installs them into the Gemini skills home", async () => {
    const home = await makeTempDir("sirius-gemini-skill-sync-");
    ...
    const after = await syncGeminiSkills(ctx, [siriusKey]);
    expect(after.entries.find((entry) => entry.key === siriusKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".gemini", "skills", "sirius"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled Sirius EcoSystem skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("sirius-gemini-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncGeminiSkills(configuredCtx, [siriusKey]);
