...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "sirius", LOGNAME: "sirius", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "sirius" }),
      },
    );

    expect(tokens).toEqual(["sirius", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "sirius" }, {
        userInfo: () => ({ username: "sirius" }),
      });

      expect(tokens).toEqual(["sirius", "custom-token"]);
    } finally {
