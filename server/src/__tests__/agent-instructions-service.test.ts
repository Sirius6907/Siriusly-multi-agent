...
describe("agent instructions service", () => {
  const originalSiriusHome = process.env.SIRIUS_HOME;
  const originalSiriusInstanceId = process.env.SIRIUS_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalSiriusHome === undefined) delete process.env.SIRIUS_HOME;
    else process.env.SIRIUS_HOME = originalSiriusHome;
    if (originalSiriusInstanceId === undefined) delete process.env.SIRIUS_INSTANCE_ID;
    else process.env.SIRIUS_INSTANCE_ID = originalSiriusInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const siriusHome = await makeTempDir("sirius-agent-instructions-home-");
    const externalRoot = await makeTempDir("sirius-agent-instructions-external-");
    cleanupDirs.add(siriusHome);
    cleanupDirs.add(externalRoot);
    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("sirius-agent-instructions-new-external-");
    cleanupDirs.add(siriusHome);
    cleanupDirs.add(externalRoot);
    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("sirius-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const siriusHome = await makeTempDir("sirius-agent-instructions-recover-");
    cleanupDirs.add(siriusHome);
    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const siriusHome = await makeTempDir("sirius-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("sirius-agent-instructions-stale-root-");
    cleanupDirs.add(siriusHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const siriusHome = await makeTempDir("sirius-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("sirius-agent-instructions-heal-write-stale-");
    cleanupDirs.add(siriusHome);
    cleanupDirs.add(staleRoot);
    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const siriusHome = await makeTempDir("sirius-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("sirius-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(siriusHome);
    cleanupDirs.add(staleRoot);
    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const siriusHome = await makeTempDir("sirius-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("sirius-agent-instructions-partial-root-");
    cleanupDirs.add(siriusHome);
    cleanupDirs.add(staleRoot);
    process.env.SIRIUS_HOME = siriusHome;
    process.env.SIRIUS_INSTANCE_ID = "test-instance";
