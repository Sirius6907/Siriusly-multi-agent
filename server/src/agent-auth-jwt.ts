...
function jwtConfig() {
  const secret = process.env.SIRIUS_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.SIRIUS_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.SIRIUS_AGENT_JWT_ISSUER ?? "sirius",
    audience: process.env.SIRIUS_AGENT_JWT_AUDIENCE ?? "sirius-api",
  };
}
