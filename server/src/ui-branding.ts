const FAVICON_BLOCK_START = "<!-- SIRIUS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- SIRIUS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- SIRIUS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- SIRIUS_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.SIRIUS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.SIRIUS_WORKTREE_NAME) ?? nonEmpty(env.SIRIUS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.SIRIUS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="sirius-worktree-enabled" content="true" />',
    `<meta name="sirius-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="sirius-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="sirius-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
