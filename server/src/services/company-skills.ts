...
import { companySkills } from "@siriusly/db";
import { readSiriusSkillSyncPreference, writeSiriusSkillSyncPreference } from "@siriusly/adapter-utils/server-utils";
import type { SiriusSkillEntry } from "@siriusly/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@siriusly/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveSiriusInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("siriusly/sirius/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveSiriusInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "sirius_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Sirius EcoSystem skills are read-only.",
      sourceLabel: "Sirius EcoSystem bundled",
      sourceBadge: "sirius",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Sirius EcoSystem workspace",
        sourceBadge: "sirius",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "sirius_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Sirius EcoSystem skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "sirius_bundled"
        && incomingKind === "github"
        && incomingOwner === "siriusly"
        && incomingRepo === "sirius"
      ) {
        out.push(existing);
        continue;
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeSiriusSkillSyncPreference(config, filtered),
        });
      }
    }
