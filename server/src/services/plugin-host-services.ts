...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@siriusly/db";
 ...
 *
 * This implementation delegates to the core Sirius EcoSystem domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@siriusly/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
