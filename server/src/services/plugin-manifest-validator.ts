 ...
 */
import { pluginManifestV1Schema } from "@siriusly/shared";
import type { SiriusPluginManifestV1 } from "@siriusly/shared";
import { PLUGIN_API_VERSION } from "@siriusly/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): SiriusPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as SiriusPluginManifestV1,
        };
      }
