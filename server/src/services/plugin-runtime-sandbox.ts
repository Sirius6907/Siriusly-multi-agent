...
import type { SiriusPluginManifestV1 } from "@siriusly/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __sirius_exports: module.exports,
      __sirius_module: module,
      __sirius_require: requireInSandbox,
      __sirius_filename: realPath,
      __sirius_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__sirius_exports, __sirius_module, __sirius_require, __sirius_filename, __sirius_dirname)`;
