 ...
 *    (`~/.sirius/plugins/`) and `node_modules` for packages matching
 *    the `sirius-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Sirius EcoSystem plugins.
 * Packages matching this pattern are considered SiriusEcoSystem plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "sirius-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: SiriusPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.sirius/plugins/ local directory
  | "npm"               // npm packages matching sirius-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: SiriusPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the sirius-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "sirius-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: SiriusPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Sirius EcoSystem plugins installed as npm packages in the current
   * Node.js environment matching the "sirius-plugin-*" naming convention.
   *
   ...
   * the "siriusPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Sirius EcoSystem plugin.
   * Throws if the package is a SiriusEcoSystem plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<SiriusPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: SiriusPluginManifestV1;
    newManifest: SiriusPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Sirius EcoSystem plugin naming convention.
 * Accepts both the "sirius-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @siriusly/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "siriusPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const siriusPlugin = pkgJson["siriusPlugin"];
  if (
    siriusPlugin !== null &&
    typeof siriusPlugin === "object" &&
    !Array.isArray(siriusPlugin)
  ) {
    const manifestRelPath = (siriusPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: SiriusPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumSiriusVersion;
}
 ...
 *   packageName: "sirius-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: SiriusPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as SiriusPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Sirius EcoSystem plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<SiriusPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Sirius EcoSystem plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasSiriusPlugin = "siriusPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasSiriusPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<SiriusPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: SiriusPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @siriusly/shared exports). Run those workers through
