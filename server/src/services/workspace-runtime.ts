...
import type { AdapterRuntimeServiceReport } from "@siriusly/adapter-utils";
import type { Db } from "@siriusly/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@siriusly/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.SIRIUS_WORKSPACE_CWD = input.worktreePath;
  env.SIRIUS_WORKSPACE_PATH = input.worktreePath;
  env.SIRIUS_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.SIRIUS_WORKSPACE_BRANCH = input.branchName;
  env.SIRIUS_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.SIRIUS_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.SIRIUS_WORKSPACE_SOURCE = input.base.source;
  env.SIRIUS_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.SIRIUS_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.SIRIUS_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.SIRIUS_PROJECT_ID = input.base.projectId ?? "";
  env.SIRIUS_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.SIRIUS_AGENT_ID = input.agent.id ?? "";
  env.SIRIUS_AGENT_NAME = input.agent.name;
  env.SIRIUS_COMPANY_ID = input.agent.companyId;
  env.SIRIUS_ISSUE_ID = input.issue?.id ?? "";
  env.SIRIUS_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.SIRIUS_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.SIRIUS_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.SIRIUS_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.SIRIUS_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.SIRIUS_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.SIRIUS_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.SIRIUS_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.SIRIUS_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.SIRIUS_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.SIRIUS_PROJECT_ID = input.workspace.projectId ?? "";
  env.SIRIUS_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.SIRIUS_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".sirius", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Sirius EcoSystem", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
