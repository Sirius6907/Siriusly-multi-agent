...
import type { BillingType, ExecutionWorkspace, ExecutionWorkspaceConfig } from "@siriusly/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_siriusWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__sirius_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function evaluateSessionCompaction(input: {
    ...
    const handoffMarkdown = [
      "Sirius EcoSystem session handoff:",
  ...
  async function executeRun(runId: string) {
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      siriusRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.siriusWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.siriusRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.siriusRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.siriusSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.siriusSessionRotationReason = sessionCompaction.reason;
      context.siriusPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.siriusSessionHandoffMarkdown;
      delete context.siriusSessionRotationReason;
      delete context.siriusPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.siriusRuntimeServices = runtimeServices;
        context.siriusRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[sirius] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected SIRIUS_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.siriusRuntimeServices = combinedRuntimeServices;
        context.siriusRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[sirius] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
