...
import type { Db } from "@siriusly/db";
import type {
...
import {
  readSiriusSkillSyncPreference,
  writeSiriusSkillSyncPreference,
} from "@siriusly/adapter-utils/server-utils";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".sirius.yaml" || normalized === ".sirius.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const sirius = isPlainRecord(metadata?.sirius) ? metadata?.sirius as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.siriusSkillKey)
    ?? asString(sirius?.skillKey)
    ?? asString(sirius?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "sirius_bundled") {
    return `siriusly/sirius/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .sirius.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .sirius.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .sirius.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .sirius.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Sirius EcoSystem will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .sirius.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .sirius.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[siriusExtensionPath];
  if (selectedFiles.has(siriusExtensionPath) && typeof extensionEntry === "string") {
    filtered[siriusExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findSiriusExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".sirius.yaml"] === "string") return ".sirius.yaml";
  if (typeof files[".sirius.yml"] === "string") return ".sirius.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.sirius.yaml") || entry.endsWith("/.sirius.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "sirius_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "siriusly/sirius",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/siriusly/sirius/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "sirius_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.siriusSkillKey = skill.key;
  metadata.sirius = {
    ...(isPlainRecord(metadata.sirius) ? metadata.sirius : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const siriusExtensionPath = findSiriusExtensionPath(normalizedFiles);
  const siriusExtension = siriusExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, siriusExtensionPath) ?? "")
    : {};
  const siriusCompany = isPlainRecord(siriusExtension.company) ? siriusExtension.company : {};
  const siriusSidebar = normalizePortableSidebarOrder(siriusExtension.sidebar);
  const siriusAgents = isPlainRecord(siriusExtension.agents) ? siriusExtension.agents : {};
  const siriusProjects = isPlainRecord(siriusExtension.projects) ? siriusExtension.projects : {};
  const siriusTasks = isPlainRecord(siriusExtension.tasks) ? siriusExtension.tasks : {};
  const siriusRoutines = isPlainRecord(siriusExtension.routines) ? siriusExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(siriusCompany.brandColor),
      logoPath: asString(siriusCompany.logoPath) ?? asString(siriusCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof siriusCompany.requireBoardApprovalForNewAgents === "boolean"
          ? siriusCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
    },
    sidebar: siriusSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(siriusAgents[slug]) ? siriusAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(siriusProjects[slug]) ? siriusProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(siriusTasks[slug]) ? siriusTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(siriusRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(siriusRoutines[slug]) ? siriusRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const siriusAgentsOut: Record<string, Record<string, unknown>> = {};
    const siriusProjectsOut: Record<string, Record<string, unknown>> = {};
    const siriusTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const siriusRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readSiriusSkillSyncPreference(
        ...
        siriusAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      siriusProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      siriusTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      siriusRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const siriusExtensionPath = ".sirius.yaml";
    const siriusAgents = Object.fromEntries(
      Object.entries(siriusAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const siriusProjects = Object.fromEntries(
      Object.entries(siriusProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const siriusTasks = Object.fromEntries(
      Object.entries(siriusTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const siriusRoutines = Object.fromEntries(
      Object.entries(siriusRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[siriusExtensionPath] = buildYamlFile(
      {
        schema: "sirius/v1",
        ...
        agents: Object.keys(siriusAgents).length > 0 ? siriusAgents : undefined,
        projects: Object.keys(siriusProjects).length > 0 ? siriusProjects : undefined,
        tasks: Object.keys(siriusTasks).length > 0 ? siriusTasks : undefined,
        routines: Object.keys(siriusRoutines).length > 0 ? siriusRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, siriusExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeSiriusSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
