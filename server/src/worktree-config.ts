...
import type { SiriusConfig } from "@siriusly/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Sirius EcoSystem worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.SIRIUS_IN_WORKTREE !== "true") return null;

  const configPath = resolveSiriusConfigPath(overrideConfigPath);
  const envPath = resolveSiriusEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.SIRIUS_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.SIRIUS_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.SIRIUS_HOME) ??
      nonEmpty(env.SIRIUS_WORKTREES_DIR) ??
      "~/.sirius-worktrees",
  );
...
function writeConfigFile(configPath: string, config: SiriusConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".sirius", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as SiriusConfig;
...
): { config: SiriusConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.SIRIUS_HOME = context.homeDir;
  process.env.SIRIUS_INSTANCE_ID = context.instanceId;
  process.env.SIRIUS_CONFIG = context.configPath;
  process.env.SIRIUS_CONTEXT = context.contextPath;
  process.env.SIRIUS_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SiriusConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    SIRIUS_HOME: context.homeDir,
    SIRIUS_INSTANCE_ID: context.instanceId,
    SIRIUS_CONFIG: context.configPath,
    SIRIUS_CONTEXT: context.contextPath,
    SIRIUS_IN_WORKTREE: "true",
    SIRIUS_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: SiriusConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SiriusConfig;
  } catch {
    return;
  }
