...
import { agents as agentsTable, companies, heartbeatRuns } from "@siriusly/db";
...
import { runClaudeLogin } from "@siriusly/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@siriusly/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@siriusly/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@siriusly/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@siriusly/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.SIRIUS_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeSiriusSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readSiriusSkillSyncPreference(
