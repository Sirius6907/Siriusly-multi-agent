...
import type { DeploymentExposure, DeploymentMode } from "@siriusly/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "sirius" &&
    normalized !== "sirius-create-agent" &&
    normalized !== "sirius-create-plugin" &&
...
/** Resolve the Sirius EcoSystem repo skills directory (built-in / managed skills). */
function resolveSiriusSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const siriusSkillsDir = resolveSiriusSkillsDir();

  // Build set of Sirius EcoSystem-managed skill names
  const siriusSkillNames = new Set<string>();
  if (siriusSkillsDir) {
    try {
      for (const entry of fs.readdirSync(siriusSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) siriusSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isSiriusManaged: siriusSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.siriusApiUrl)) {
    const legacySiriusApiUrl = nonEmptyTrimmedString(input.siriusApiUrl);
    if (legacySiriusApiUrl) merged.siriusApiUrl = legacySiriusApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    siriusApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.siriusApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawSiriusApiUrl =
    typeof defaults.siriusApiUrl === "string"
      ? defaults.siriusApiUrl.trim()
      : "";
  if (rawSiriusApiUrl) {
    try {
      const parsedSiriusApiUrl = new URL(rawSiriusApiUrl);
      if (
        parsedSiriusApiUrl.protocol !== "http:" &&
        parsedSiriusApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_sirius_api_url_protocol",
          level: "warn",
          message: `siriusApiUrl must use http:// or https:// (got ${parsedSiriusApiUrl.protocol}).`
        });
      } else {
        normalized.siriusApiUrl = parsedSiriusApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_sirius_api_url_configured",
          level: "info",
          message: `siriusApiUrl set to ${parsedSiriusApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_sirius_api_url_invalid",
        level: "warn",
        message: `Invalid siriusApiUrl: ${rawSiriusApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Sirius EcoSystem host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Sirius EcoSystem is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm siriusly allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/sirius";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/sirius-claimed-api-key.json and load SIRIUS_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: siriusApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Sirius EcoSystem hostname is reachable and allowed via `pnpm siriusly allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Sirius EcoSystem API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/sirius/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Sirius EcoSystem OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          siriusApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Sirius EcoSystem generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "siriusApiUrl": "https://sirius-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Sirius EcoSystem before key claim is allowed.
    ...
    - ~/.openclaw/workspace/sirius-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/sirius-claimed-api-key.json

    And set the SIRIUS_API_KEY and SIRIUS_API_URL in your environment variables as specified here:
    ...
    Then set SIRIUS_API_KEY and SIRIUS_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Sirius EcoSystem skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your SIRIUS_API_URL to the top of your skill and note the path to your SIRIUS_API_URL
    ...
    ${
      ...
      "Ensure Sirius EcoSystem is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Sirius EcoSystem base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.siriusApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm siriusly allowed-hostname <host>
      - then restart Sirius EcoSystem and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@sirius.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "sirius", path: "/api/skills/sirius" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "sirius-create-agent",
          path: "/api/skills/sirius-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              siriusApiUrl: req.body.siriusApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.siriusApiUrl &&
          !persistedDefaults.siriusApiUrl
        ) {
          missingPersistedFields.push("siriusApiUrl");
        }
