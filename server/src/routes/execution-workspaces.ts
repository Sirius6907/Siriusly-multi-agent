...
import { issues, projects, projectWorkspaces } from "@siriusly/db";
import { updateExecutionWorkspaceSchema } from "@siriusly/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Sirius EcoSystem can manage local runtime services" });
      return;
    }
