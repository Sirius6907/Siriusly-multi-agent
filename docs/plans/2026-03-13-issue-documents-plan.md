...
Add first-class **documents** to Sirius EcoSystem as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Sirius EcoSystem workflow/docs.
...
Update the Sirius EcoSystem agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/sirius/SKILL.md`
