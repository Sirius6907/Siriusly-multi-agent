...
Audience: Operators setting up and running agents in Sirius EcoSystem

## 1. What this system does

Agents in Sirius EcoSystem do not run continuously.  
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`), Sirius EcoSystem assumes the CLI is already installed and authenticated on the host machine.
...
Sirius EcoSystem stores session IDs for resumable adapters.
...
Sirius EcoSystem pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Sirius EcoSystem surfaces this as a warning in environment tests, not a hard error.
