...
Sirius EcoSystem companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .sirius.yaml     # Adapter config, env inputs, routines
```
...
- **.sirius.yaml** holds Sirius EcoSystem-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
siriusly company export <company-id> --out ./my-export
```
...
siriusly company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
siriusly company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
siriusly company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.sirius.yaml`
...
```sh
# From a local folder
siriusly company import ./my-export

# From a GitHub URL
siriusly company import https://github.com/org/repo

# From a GitHub subfolder
siriusly company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
siriusly company import org/repo
siriusly company import org/repo/companies/acme
```
...
If `--target` is not specified, Sirius EcoSystem infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
siriusly company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
siriusly company import org/company-templates/engineering-team \
  --target new \
...
```sh
siriusly company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
siriusly company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
siriusly company import ./package \
  --target new \
  --yes \
  --json
```
...
Sirius EcoSystem supports several GitHub URL formats:
