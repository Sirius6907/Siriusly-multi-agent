...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Sirius EcoSystem.
...
- Sirius EcoSystem does not automatically start or stop these runtime services as part of issue execution.
- Sirius EcoSystem also does not automatically restart workspace runtime services on server boot.
...
2. Sirius EcoSystem realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Sirius EcoSystem persists execution-workspace metadata such as paths, refs, and provisioning settings.
