...
How to get OpenClaw running in a Docker container for local development and testing the Sirius EcoSystem OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Sirius EcoSystem includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-sirius-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Sirius EcoSystem host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-sirius-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `SIRIUS_HOST_PORT` (default `3100`)
- `SIRIUS_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Sirius EcoSystem deployment is `authenticated`, provide auth context:

```bash
SIRIUS_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
SIRIUS_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Sirius EcoSystem server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Sirius EcoSystem URL (typically `http://host.docker.internal:3100`).
- If Sirius EcoSystem rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm siriusly allowed-hostname host.docker.internal
```

Then restart Sirius EcoSystem and rerun the smoke script.
