...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Sirius EcoSystem.
...
- be useful outside Sirius EcoSystem
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.sirius.yaml`
...
Example Sirius EcoSystem extension:
...
- Sirius EcoSystem must not require extra top-level fields for skill validity
- Sirius EcoSystem-specific extensions must live under `metadata.sirius` or `metadata.sources`
...
For Sirius EcoSystem, the preferred fidelity extension is:
...
- issue/task Sirius EcoSystem-only metadata
...
- Sirius EcoSystem tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Sirius EcoSystem shape:
...
Additional rules for Sirius EcoSystem exporters:
...
- omit empty and default-valued Sirius EcoSystem fields when possible
...
- Sirius EcoSystem exporters should emit `.sirius.yaml` as a sidecar by default
...
## 19. Sirius EcoSystem Mapping

Sirius EcoSystem can map this spec to its runtime model like this:
...
- Sirius EcoSystem extension:
  - `.sirius.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Sirius EcoSystem-specific fidelity

Inline Sirius EcoSystem-only metadata that must live inside a shared markdown file should use:

- `metadata.sirius`

That keeps the base format broader than Sirius EcoSystem.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Sirius EcoSystem.

## 20. Cutover

Sirius EcoSystem should cut over to this markdown-first package model as the primary portability format.

`sirius.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Sirius EcoSystem, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
