# ClipHub: Marketplace for Sirius EcoSystem Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Sirius EcoSystem configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Sirius EcoSystem-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Sirius EcoSystem company (agents, projects, governance) |
...
A complete Sirius EcoSystem company configuration:
...
Single-agent configurations designed to plug into a Sirius EcoSystem org:
...
Portable skill files that any Sirius EcoSystem agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Sirius EcoSystem's skill loading system
...
interface Listing {
  ...
  siriusVersionMin: string;     // Minimum Sirius EcoSystem version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Sirius EcoSystem company
...
| `POST` | `/api/listings/:id/install` | Install to Sirius EcoSystem company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Sirius EcoSystem company (or create new)
  → ClipHub API calls Sirius EcoSystem API to:
  ...
  → Redirect to Sirius EcoSystem dashboard with new team running
```
...
### 5.3 Creator: Export from Sirius EcoSystem → Publish

```
Running Sirius EcoSystem company → "Export as Blueprint" (CLI or UI)
  → Sirius EcoSystem exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Sirius EcoSystem brand blue/purple)
...
| Install flow | One-click deploy to Sirius EcoSystem company |
...
{
  "targetCompanyId": "uuid",         // Existing Sirius EcoSystem company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `sirius-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Sirius EcoSystem
- **Backend**: Node.js API (or extend Sirius EcoSystem server)
- **Database**: Postgres (can share Sirius EcoSystem's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Sirius EcoSystem auth (or OAuth2)

### 9.2 Integration with Sirius EcoSystem

ClipHub can be:
- **Option A**: A separate app that calls Sirius EcoSystem's API to install blueprints
- **Option B**: A built-in section of the Sirius EcoSystem UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Sirius EcoSystem UI and API.
...
- [ ] Install flow: blueprint → Sirius EcoSystem company
...
- [ ] "Export from Sirius EcoSystem" CLI command
