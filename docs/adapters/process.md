...
- Running a Python script that calls the Sirius EcoSystem API
...
1. Sirius EcoSystem spawns the configured command as a child process
2. Standard Sirius EcoSystem environment variables are injected (`SIRIUS_AGENT_ID`, `SIRIUS_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Sirius EcoSystem API and perform work.
