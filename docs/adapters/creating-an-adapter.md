...
Build a custom adapter to connect Sirius EcoSystem to any agent runtime.
...
2. Build environment with `buildSiriusEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `siriusly run --watch` using `picocolors`.
...
Make Sirius EcoSystem skills discoverable to your agent runtime without writing to the agent's working directory:
