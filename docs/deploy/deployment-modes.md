...
Sirius EcoSystem supports two runtime modes with different security profiles.
...
```sh
pnpm siriusly allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Sirius EcoSystem emits a one-time claim URL at startup:
...
```sh
SIRIUS_DEPLOYMENT_MODE=authenticated pnpm siriusly run
```
