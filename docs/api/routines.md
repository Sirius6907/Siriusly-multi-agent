...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Sirius EcoSystem-Signature` + `X-Sirius EcoSystem-Timestamp` header pair matching the trigger's signing mode.
