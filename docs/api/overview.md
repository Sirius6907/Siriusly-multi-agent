...
Sirius EcoSystem exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`SIRIUS_API_KEY`)
...
- Run audit trail: include `X-Sirius EcoSystem-Run-Id` header on all mutating requests during heartbeats
