...
Sirius EcoSystem is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── sirius/               # Core Sirius EcoSystem skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Sirius EcoSystem env vars and a prompt
4. **Agent work** — The agent calls Sirius EcoSystem's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Sirius EcoSystem and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `siriusly run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Sirius EcoSystem orchestrates agents; it doesn't run them
