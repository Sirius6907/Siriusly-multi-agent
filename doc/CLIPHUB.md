...
ClipHub is the public registry where people share, discover, and download Sirius EcoSystem company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Sirius EcoSystem what a package registry is to a programming language. Sirius EcoSystem already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Sirius EcoSystem — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Sirius EcoSystem instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Sirius EcoSystem spec. It contains:
...
**Install (fresh start):**
```
sirius install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Sirius EcoSystem instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Sirius EcoSystem, export your company as a template, then publish:

```
sirius export --template my-company
sirius publish cliphub my-company
```
...
ClipHub is a **separate service** from Sirius EcoSystem itself. Sirius EcoSystem is self-hosted; ClipHub is a hosted registry that Sirius EcoSystem instances talk to.
...
| **Sirius EcoSystem CLI** | `siriusly install`, `siriusly publish`, `siriusly cliphub sync` — built into Sirius EcoSystem |
| **Sirius EcoSystem UI** | "Browse ClipHub" panel in the Sirius EcoSystem web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Sirius EcoSystem) |
| Backend | TypeScript + Hono (consistent with Sirius EcoSystem) |
| Database | PostgreSQL |
...
4. Run `siriusly install cliphub:acme/lean-saas-shop`
5. Sirius EcoSystem creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Sirius EcoSystem until it works well
2. Export: `siriusly export --template my-agency`
3. Publish: `siriusly publish cliphub my-agency`
...
3. Install just that agent: `siriusly install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Sirius EcoSystem

ClipHub is **not required** to use Sirius EcoSystem. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Sirius EcoSystem what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `siriusly install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Sirius EcoSystem web UI
- [ ] `siriusly cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Sirius EcoSystem's own philosophy)
