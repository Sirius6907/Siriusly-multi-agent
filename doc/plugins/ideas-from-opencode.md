...
Sirius EcoSystem V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Sirius EcoSystem-shaped design.

Assumption for this document: Sirius EcoSystem is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Sirius EcoSystem objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Sirius EcoSystem.

The main conclusion is:

- Sirius EcoSystem should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Sirius EcoSystem should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Sirius EcoSystem should use multiple extension classes instead of one generic plugin bag:
...
If Sirius EcoSystem does this well, the examples you listed become straightforward:
...
Relevant Sirius EcoSystem files reviewed for current extension seams:
...
It is too dangerous for Sirius EcoSystem core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Sirius EcoSystem — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Sirius EcoSystem should absolutely do this.
...
Sirius EcoSystem should copy this discipline.
...
Sirius EcoSystem should adopt this style for plugin actions, automations, and UI schemas.
...
Sirius EcoSystem can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Sirius EcoSystem too.

## What Sirius EcoSystem Should Not Copy Directly
...
Sirius EcoSystem is a control plane for an operator-managed instance with company objects.
...
Sirius EcoSystem is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Sirius EcoSystem should not auto-load arbitrary code from a workspace repo like `.sirius/plugins` or project directories.
...
For Sirius EcoSystem, equivalent hooks into:
...
Sirius EcoSystem should not allow plugins to silently replace:
...
For Sirius EcoSystem it would be risky because it combines:
...
Sirius EcoSystem should require an explicit operator install step.

## Why Sirius EcoSystem Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Sirius EcoSystem |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Sirius EcoSystem instance |
...
That means Sirius EcoSystem should borrow the good ideas from `opencode` but use a stricter architecture.

## Sirius EcoSystem Already Has Useful Pre-Plugin Seams

Sirius EcoSystem has several extension-like seams already:
...
This is good news.
Sirius EcoSystem does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Sirius EcoSystem Plugin Model
...
Sirius EcoSystem already has this pattern implicitly:
...
- `plugin` means integration code that talks to Sirius EcoSystem through a typed plugin protocol and capability model
...
- invoke explicit Sirius EcoSystem actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@siriusly/plugin-sdk/ui`) with design tokens and common components.
...
Sirius EcoSystem should treat plugin installation as a global instance-level action.

Examples:

- install `@sirius/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Sirius EcoSystem objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Sirius EcoSystem already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Sirius EcoSystem too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@sirius/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@siriusly/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Sirius EcoSystem instance.

A `create-sirius-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Sirius EcoSystem achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Sirius EcoSystem server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Sirius EcoSystem's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@siriusly/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@siriusly/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Sirius EcoSystem

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@siriusly/plugin-sdk";

export default definePlugin({
  id: "@sirius/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Sirius EcoSystem entities
    });
    ...
    ctx.events.on("plugin.@sirius/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@siriusly/plugin-sdk/ui";
...
Any Sirius EcoSystem plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Sirius EcoSystem objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@sirius/plugin-linear`)
...
- reuse core tables when the data is clearly part of Sirius EcoSystem itself
...
If a concept is becoming part of Sirius EcoSystem's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Sirius EcoSystem itself
...
### 3. Scope plugin data to Sirius EcoSystem objects before adding custom schemas

A lot of plugin data naturally hangs off existing Sirius EcoSystem objects:
...
If Sirius EcoSystem eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@sirius/plugin-workspace-files`
...
Package idea: `@sirius/plugin-terminal`
...
Package idea: `@sirius/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Sirius EcoSystem.
- Board reviews the diff after a run without leaving Sirius EcoSystem.
...
- optional `events.emit` (e.g. `plugin.@sirius/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@sirius/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@sirius/plugin-linear`

This plugin syncs Sirius EcoSystem work with Linear. It is useful for:

- importing backlog from Linear
- linking Sirius EcoSystem issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Sirius EcoSystem and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Sirius EcoSystem.
- Sirius EcoSystem issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@sirius/plugin-github-issues`

This plugin syncs Sirius EcoSystem issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Sirius EcoSystem issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Sirius EcoSystem task governance
...
- Plugin settings:
  ...
  - whether new Sirius EcoSystem issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Sirius EcoSystem issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Sirius EcoSystem.
- GitHub webhooks update status/comment state in Sirius EcoSystem.
- A PR is linked back to the Sirius EcoSystem issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@sirius/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@sirius/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Sirius EcoSystem. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Sirius EcoSystem issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Sirius EcoSystem plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Sirius EcoSystem dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Sirius EcoSystem issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Sirius EcoSystem core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@sirius/plugin-runtime-processes`
...
Package idea: `@sirius/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Sirius EcoSystem. It is useful for:
...
- Failed payment spikes or churn events can generate Sirius EcoSystem issues for follow-up.
...
- Stripe data should stay additive to Sirius EcoSystem core
...
- plugin UI bundle loading, host bridge, `@siriusly/plugin-sdk/ui`
...
- `@siriusly/plugin-test-harness` and `create-sirius-plugin` starter template
...
Sirius EcoSystem should not implement "an OpenCode-style generic in-process hook system."
Sirius EcoSystem should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Sirius EcoSystem
...
8. Ship `@siriusly/plugin-test-harness` and `create-sirius-plugin` starter template.
