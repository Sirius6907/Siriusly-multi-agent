...
This guide describes the current, implemented way to create a Sirius EcoSystem plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Sirius EcoSystem app.
...
```bash
pnpm --filter @siriusly/create-sirius-plugin build
node packages/plugins/create-sirius-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Sirius EcoSystem repo:

```bash
pnpm --filter @siriusly/create-sirius-plugin build
node packages/plugins/create-sirius-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/sirius/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@siriusly/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@siriusly/plugin-sdk` from the local Sirius EcoSystem checkout into a `.sirius-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Sirius EcoSystem from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@siriusly/plugin-sdk/ui`
