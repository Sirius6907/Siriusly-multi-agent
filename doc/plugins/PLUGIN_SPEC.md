# Sirius EcoSystem Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Sirius EcoSystem's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Sirius EcoSystem app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Sirius EcoSystem HTTP APIs directly.
...
Sirius EcoSystem plugin design is based on the following assumptions:

1. Sirius EcoSystem is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Sirius EcoSystem business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Sirius EcoSystem core.
...
2. Let plugins add major capabilities without editing Sirius EcoSystem core.
...
4. Depend on project-local plugin folders such as `.sirius/plugins`.
...
The single Sirius EcoSystem deployment an operator installs and controls.

### 5.2 Company

A first-class Sirius EcoSystem business object inside the instance.
...
A trusted in-process extension loaded directly by Sirius EcoSystem core.
...
An installable instance-wide extension package loaded through the Sirius EcoSystem plugin runtime.
...
Sirius EcoSystem has two extension classes.
...
Sirius EcoSystem already has a concrete workspace model:
...
Plugins live under the Sirius EcoSystem instance directory.

Suggested layout:

- `~/.sirius/instances/default/plugins/package.json`
- `~/.sirius/instances/default/plugins/node_modules/`
- `~/.sirius/instances/default/plugins/.cache/`
- `~/.sirius/instances/default/data/plugins/<plugin-id>/`
...
Sirius EcoSystem should add CLI commands:

- `pnpm siriusly plugin list`
- `pnpm siriusly plugin install <package[@version]>`
- `pnpm siriusly plugin uninstall <plugin-id>`
- `pnpm siriusly plugin upgrade <plugin-id> [version]`
- `pnpm siriusly plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@sirius/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface SiriusPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Sirius EcoSystem agents can use during runs.
...
- Sirius EcoSystem server starts one worker process per installed plugin
...
Receives one typed Sirius EcoSystem domain event.
...
export function definePlugin(definition: PluginDefinition): SiriusPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: SiriusPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@siriusly/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@siriusly/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@siriusly/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@siriusly/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@siriusly/plugin-sdk/ui`
...
The `@siriusly/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Sirius EcoSystem secret provider system rather than a plain text input
...
1. Core Sirius EcoSystem data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Sirius EcoSystem objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Sirius EcoSystem product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Sirius EcoSystem later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Sirius EcoSystem secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm siriusly plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Sirius EcoSystem server. This is a normative requirement, not optional.
...
### 27.1 `@siriusly/plugin-test-harness`
...
```ts
import { createTestHarness } from "@siriusly/plugin-test-harness";
...
For developing a plugin against a running Sirius EcoSystem instance:

- The operator installs the plugin from a local path: `pnpm siriusly plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-sirius-plugin`) that scaffolds:

- `package.json` with correct `siriusPlugin` keys
...
- `@sirius/plugin-workspace-files`
- `@sirius/plugin-terminal`
- `@sirius/plugin-git`
- `@sirius/plugin-linear`
- `@sirius/plugin-github-issues`
- `@sirius/plugin-grafana`
- `@sirius/plugin-runtime-processes`
- `@sirius/plugin-stripe`
...
- `@siriusly/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@siriusly/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@siriusly/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@siriusly/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@siriusly/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@siriusly/plugin-sdk/ui`
...
- `@siriusly/plugin-test-harness`
- `create-sirius-plugin` starter template
...
Sirius EcoSystem should not implement a generic in-process hook bag modeled directly after local coding tools.

Sirius EcoSystem should implement:
...
That is the complete target design for the Sirius EcoSystem plugin system.
