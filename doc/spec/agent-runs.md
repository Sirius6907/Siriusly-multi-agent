...
This spec defines how Sirius EcoSystem actually runs agents while staying runtime-agnostic.
...
1. Sirius EcoSystem is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Sirius EcoSystem manages lifecycle, and on exit Sirius EcoSystem parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `sirius.skill` (shared Sirius EcoSystem skill text block)
...
2. Preferred transport is env vars (`SIRIUS_*`) injected at runtime.
