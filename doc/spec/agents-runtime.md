...
For `claude_local` and `codex_local`, Sirius EcoSystem assumes the CLI is already installed and authenticated on the host machine.
...
Sirius EcoSystem stores resumable session state per `(agent, taskKey, adapterType)`.
