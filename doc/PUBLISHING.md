...
Low-level reference for how Sirius EcoSystem packages are prepared and published to npm.
...
Sirius EcoSystem no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `siriusly`, imports code from workspace packages such as:
...
### Manual first publish for `@siriusly/ui`
...
npm view @siriusly/ui version

# make sure the dist payload is fresh
pnpm --filter @siriusly/ui build
...
- If `npm view @siriusly/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@siriusly/ui`, the `siriusly` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Sirius EcoSystem uses calendar versions:
