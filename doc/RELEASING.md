# Releasing Sirius EcoSystem

Maintainer runbook for shipping Sirius EcoSystem across npm, GitHub, and the website-facing changelog surface.
...
Sirius EcoSystem uses calendar versions that still fit semver syntax:
...
2. **npm** — `siriusly` and public workspace packages are published
...
```bash
npx siriusly@canary onboard
# or
npx siriusly@canary onboard --data-dir "$(mktemp -d /tmp/sirius-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/siriusly/sirius/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Sirius EcoSystem. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
SIRIUS_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
SIRIUS_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary SIRIUS_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable SIRIUS_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f sirius_version=canary
gh workflow run release-smoke.yml -f sirius_version=latest
```

Minimum checks:

- `npx siriusly@canary onboard` installs
