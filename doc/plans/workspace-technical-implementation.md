...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/sirius-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `SIRIUS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `SIRIUS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Sirius EcoSystem may create host-local worktrees, processes, and paths
- `cloud`
  - Sirius EcoSystem should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `sirius | github | vercel | s3 | custom`
...
- use `SIRIUS_EXECUTION_TOPOLOGY` as a defaulting guardrail
