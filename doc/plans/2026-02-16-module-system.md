# Sirius EcoSystem Module System
...
Sirius EcoSystem's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Sirius EcoSystem's API, UI, and data model | Yes |
...
modules/
  observability/
    sirius.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@siriusly/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@siriusly/core";
...
```
1. Scan modules/ directory for sirius.module.json manifests
...
```jsonc
// sirius.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@siriusly/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@siriusly/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:sirius/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.sirius.eco/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm siriusly store list                    # browse available modules and templates
pnpm siriusly store install <module-id>     # install a module
pnpm siriusly store import <template-id>    # import a company template
pnpm siriusly store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Sirius EcoSystem instance | `server:started` |
...
4. **Module config** — `sirius.config.json` with per-module config, env var interpolation
...
10. **`@siriusly/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
