...
Agents need API keys to authenticate with Sirius EcoSystem. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Sirius EcoSystem
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Sirius EcoSystem generates a token and passes it directly to the agent
...
`SIRIUS_API_URL`, `SIRIUS_AGENT_ID`, `SIRIUS_COMPANY_ID`. We need to
add a `SIRIUS_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Sirius EcoSystem CLI
...
```
sirius auth login
# Opens browser -> user confirms -> token stored at ~/.sirius/credentials
```
...
managed by the Sirius EcoSystem server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Sirius EcoSystem.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Sirius EcoSystem SKILL.md (or a link to it)
   - What information Sirius EcoSystem needs from the agent (e.g. webhook URL, adapter
...
5. Sirius EcoSystem stores the pending registration.
...
7. On approval, Sirius EcoSystem provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Sirius EcoSystem issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.sirius.eco/skills/sirius/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Sirius EcoSystem should send you heartbeats",
...
1. Generate a Sirius EcoSystem invite link for the company.
...
4. A Sirius EcoSystem company member approves the new agent.
5. Sirius EcoSystem begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `SIRIUS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `siriusly auth login` for developer-managed remote agents.                                      |
