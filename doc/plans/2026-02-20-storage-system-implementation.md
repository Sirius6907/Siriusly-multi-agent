...
Add a single storage subsystem for Sirius EcoSystem that supports:
...
- API serves attachment bytes through authenticated Sirius EcoSystem endpoints.
...
- Default local path is under instance root: `~/.sirius/instances/<instanceId>/data/storage`.
...
- `siriusly onboard` writes a valid `storage` config block by default.
- `siriusly configure --section storage` can switch between local and s3 modes.
...
- `siriusly doctor` reports actionable storage status.
