...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Sirius EcoSystem plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Sirius EcoSystem.
...
- package: `@siriusly/plugin-kitchen-sink-example`
- plugin id: `sirius.kitchen-sink-example` or `sirius-kitchen-sink-example`

Recommendation: use `sirius-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Sirius EcoSystem event such as `issue.created`
...
Recommendation: default to a Sirius EcoSystem-local endpoint or a stable public echo endpoint to avoid flaky docs.
