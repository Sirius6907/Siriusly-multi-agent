...
- New config/env settings:
  - `SIRIUS_AGENT_JWT_SECRET`
  - `SIRIUS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `SIRIUS_AGENT_JWT_ISSUER` (default: `sirius`)
  - `SIRIUS_AGENT_JWT_AUDIENCE` (default: `sirius-api`)
...
1. In:
   ...
   inject `SIRIUS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `SIRIUS_API_KEY`, do not overwrite it.
- Continue injecting:
  - `SIRIUS_AGENT_ID`
  - `SIRIUS_COMPANY_ID`
  - `SIRIUS_API_URL`
...
- Local adapters authenticate without manual `SIRIUS_API_KEY` config.
