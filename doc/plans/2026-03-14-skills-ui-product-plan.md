...
This document defines the product and UI plan for skill management in Sirius EcoSystem.
...
- Sirius EcoSystem company import/export can include skills as package content
...
3. The UI must always tell the truth about what Sirius EcoSystem knows:
   - desired state in Sirius EcoSystem
...
Sirius EcoSystem should treat skills at two scopes:
...
- created locally inside Sirius EcoSystem later
...
7. “Understand what Sirius EcoSystem knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Sirius EcoSystem-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Sirius EcoSystem did not install.

These should be clearly marked:

- external
- not managed by Sirius EcoSystem
...
- `desired_only`: Sirius EcoSystem wants it, adapter does not show it yet
- `external`: adapter has it but Sirius EcoSystem does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Sirius EcoSystem only tracks desired state
...
- Sirius EcoSystem can still track desired skills
...
- Sirius EcoSystem can see adapter skills
...
- `.sirius.yaml` may add Sirius EcoSystem-specific fidelity, but should not replace the base shortname association model
...
- `.sirius.yaml` may add Sirius EcoSystem-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Sirius EcoSystem should have a company-scoped skill model or managed package model representing:
...
Sirius EcoSystem should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Sirius EcoSystem-specific.
...
That gives Sirius EcoSystem one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
