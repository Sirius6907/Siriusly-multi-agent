...
This document defines the V1 design for a Sirius EcoSystem adapter that integrates with
...
4. synthesized stdout events for Sirius EcoSystem UI/CLI
...
3. **Agent callback auth to Sirius EcoSystem** uses a bootstrap exchange flow (no long-lived Sirius EcoSystem key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Sirius EcoSystem endpoints, not full SKILL.md prompt injection.
...
- `siriusPublicUrl` (optional override; else `SIRIUS_PUBLIC_URL` env)
...
## Sirius EcoSystem Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `SIRIUS_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.siriusPublicUrl`
2. `process.env.SIRIUS_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Sirius EcoSystem credentials in prompt text.

Flow:

1. Before launch/follow-up, Sirius EcoSystem mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Sirius EcoSystem validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Sirius EcoSystem API calls.
...
1. Prompt includes a compact instruction to fetch skills from Sirius EcoSystem.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/sirius`
   - `GET /api/skills/sirius-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <siriusPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Sirius EcoSystem run context.
...
5. `siriusPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Sirius EcoSystem skill endpoints |
| Sirius EcoSystem auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Sirius EcoSystem context.
3. Add richer PR metadata surfacing in Sirius EcoSystem UI.
