...
This document defines the next-stage plan for Sirius EcoSystem company import/export.

The core shift is:

- move from a Sirius EcoSystem-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Sirius EcoSystem.
...
Sirius EcoSystem already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Sirius EcoSystem
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `sirius.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.sirius.yaml` sidecar for high-fidelity Sirius EcoSystem-specific details
...
11. adapter-aware skill sync surfaces so Sirius EcoSystem can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Sirius EcoSystem at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Sirius EcoSystem sidecar.
...
Sirius EcoSystem must not redefine `SKILL.md`.
...
- Sirius EcoSystem-specific fidelity lives in `.sirius.yaml`
- Sirius EcoSystem may resolve and install `SKILL.md` packages, but it must not require a Sirius EcoSystem-only skill format
...
### 5.4 Base Package Vs Sirius EcoSystem Extension
...
- base package:
  ...
  - no Sirius EcoSystem-only runtime fields by default
- Sirius EcoSystem extension:
  - `.sirius.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Sirius EcoSystem tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`sirius.manifest.json` is not part of the future package direction.
...
Sirius EcoSystem import/export should support these entity kinds:
...
In Sirius EcoSystem V2 portability:
...
- Sirius EcoSystem can treat “this set of agents came from team package X” as the imported-team model
...
Sirius EcoSystem should support source references in package metadata with:
...
Sirius EcoSystem should define a new adapter surface area around skills:
...
Planned Sirius EcoSystem behavior:

- if an adapter supports read, Sirius EcoSystem should show current skills in the UI
- if an adapter supports write, Sirius EcoSystem should let the user enable/disable imported skills
- if an adapter supports sync, Sirius EcoSystem should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.sirius.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.sirius.yaml` alongside the base package
...
## 10. Storage Model Inside Sirius EcoSystem
...
Sirius EcoSystem should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `siriusly company export <company-id> --out <path>`
- `siriusly company import <path-or-url> --dry-run`
- `siriusly company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Sirius EcoSystem storage, or only referenced at import time?
...
3. Should Sirius EcoSystem support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Sirius EcoSystem aligned with:
