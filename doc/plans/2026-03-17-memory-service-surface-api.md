# Sirius EcoSystem Memory Service Plan

## Goal

Define a Sirius EcoSystem memory service and surface API that can sit above multiple memory backends, while preserving Sirius EcoSystem's control-plane requirements:

- company scoping
- auditability
- provenance back to Sirius EcoSystem work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current Sirius EcoSystem architecture in:
...
Sirius EcoSystem should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing Sirius EcoSystem plugin runtime.

That keeps the core small and matches the current direction that optional knowledge-like systems live at the edges.

### 4. Sirius EcoSystem owns routing, provenance, and accounting

Providers should not decide how Sirius EcoSystem entities map to governance.

Sirius EcoSystem core should own:
...
The normalized Sirius EcoSystem scope passed into a provider request.
...
A normalized write, query, browse, or delete action performed through Sirius EcoSystem.

Sirius EcoSystem should log every operation, whether the provider is local or external.
...
## What Sirius EcoSystem Should Persist

Sirius EcoSystem should not mirror the full provider memory corpus into Postgres unless the provider is a Sirius EcoSystem-managed local provider.

Sirius EcoSystem core should persist:
...
1. `pre-run hydrate`
   Before an agent run starts, Sirius EcoSystem may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, Sirius EcoSystem may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, Sirius EcoSystem may capture selected issue comments or issue documents as memory sources.
...
Sirius EcoSystem should give agents both automatic recall and explicit tools, with simple guidance:
...
Sirius EcoSystem needs a first-class UI for memory, otherwise providers become black boxes.
...
Sirius EcoSystem should roll up:
...
This is important because a memory system that "works" but silently burns budget is not acceptable in Sirius EcoSystem.
...
- it matches Sirius EcoSystem's local-first posture
- it is inspectable
...
- Do we want Sirius EcoSystem-managed extraction pipelines at all, or should built-ins be the only place where Sirius EcoSystem owns extraction?
...
- Sirius EcoSystem owns memory bindings, scopes, provenance, governance, and usage reporting.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives Sirius EcoSystem a stable "memory service" without locking the product to one memory philosophy or one vendor.
