...
Today release smoke testing for published Sirius EcoSystem packages is manual and shell-driven:
...
- runs `siriusly@${SIRIUS_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@sirius.local`
- password: `sirius-smoke-password`
...
1. A published `siriusly@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `siriusly@latest` Docker install can be smoke-tested by Playwright in CI.
