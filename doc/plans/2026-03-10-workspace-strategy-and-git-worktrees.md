...
`PAP-447` asks how Sirius EcoSystem should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Sirius EcoSystem instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Sirius EcoSystem:

- some operators use Sirius EcoSystem to manage Sirius EcoSystem and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Sirius EcoSystem
- Claude and Codex expose different built-in affordances, so Sirius EcoSystem should not overfit to one tool

## Core Product Decision

Sirius EcoSystem should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Sirius EcoSystem
...
- Sirius EcoSystem should not assume that every adapter can see or use a host filesystem path directly
...
If Sirius EcoSystem makes worktrees agent-first, it will blur:
...
- Sirius EcoSystem core resolves and records execution workspace state
...
That gives Sirius EcoSystem room to support:
...
- Sirius EcoSystem handles workspace runtime services under the hood using project/workspace policy
...
Once Sirius EcoSystem is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Sirius EcoSystem should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Sirius EcoSystem cannot infer
...
- when issue moves to done, should Sirius EcoSystem auto-commit?
- should it auto-push?
...
Sirius EcoSystem already has the right foundation for a project-first model.
...
For server-spawned local adapters, Sirius EcoSystem already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Sirius EcoSystem should mirror directly

Implication:

- for `codex_local`, Sirius EcoSystem should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Sirius EcoSystem should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Sirius EcoSystem should separate:
...
Sirius EcoSystem should be able to express intentions such as:
...
These are not specific to Sirius EcoSystem. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".sirius/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Sirius EcoSystem-specific `devServer` flag.
...
Sirius EcoSystem should distinguish between:
...
Sirius EcoSystem should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
### Sirius EcoSystem Core

Sirius EcoSystem core should:
...
Sirius EcoSystem core should not:
...
In addition to the local git helper, Sirius EcoSystem should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Sirius EcoSystem
...
For runtime services specifically, Sirius EcoSystem should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Sirius EcoSystem-only dev-server flags
...
3. Inject resolved execution workspace details into `context.siriusWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Sirius EcoSystem instance can be reused through env/config injection when appropriate
...
- Sirius EcoSystem can decide whether to reuse or start a fresh service deterministically
...
3. Test against Sirius EcoSystem-on-Sirius EcoSystem workflows first.
...
7. Workspace runtime services are modeled generically, not as Sirius EcoSystem-specific dev-server toggles.
