...
Sirius EcoSystem needs to support two very different but equally valid ways of working:
...
Today, Sirius EcoSystem already has the beginnings of this model:
...
- making Sirius EcoSystem feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Sirius EcoSystem users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Sirius EcoSystem deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Sirius EcoSystem into a full code review product
...
This object must be recorded explicitly so that Sirius EcoSystem can:
...
Sirius EcoSystem should treat PRs as a type of work product linked back to:
...
Sirius EcoSystem already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Sirius EcoSystem must be a first-class deployment mode

Sirius EcoSystem cannot assume that it is running on the same machine as the code.

In cloud deployments, Sirius EcoSystem may:
...
- `Runtime service`: a process or service Sirius EcoSystem owns or tracks for a workspace
...
It also lets cloud-hosted Sirius EcoSystem deployments point at codebases and remotes without pretending the Sirius EcoSystem host has direct filesystem access.
...
This lets Sirius EcoSystem support:
...
Without an explicit `execution workspace` record, Sirius EcoSystem has nowhere stable to attach:
...
Sirius EcoSystem needs a single place to show:
...
- Sirius EcoSystem-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Sirius EcoSystem owning the execution host
...
## 1. Sirius EcoSystem host and execution host must be decoupled

Sirius EcoSystem may run:
...
without the Sirius EcoSystem host itself running local git or local preview processes.
...
Sirius EcoSystem should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Sirius EcoSystem.
...
Sirius EcoSystem should keep workspace automation centered on a small set of hooks:
...
Sirius EcoSystem should provide the equivalent visibility in the project `Code` surface:
...
Sirius EcoSystem should treat execution workspaces as the natural home for:
...
Sirius EcoSystem should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Sirius EcoSystem should continue treating cleanup policy as part of the core workspace model:
...
Sirius EcoSystem should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Sirius EcoSystem host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Sirius EcoSystem should borrow Worktrunk's good execution patterns while keeping the broader Sirius EcoSystem model:
...
This is how Sirius EcoSystem supports:
...
That keeps Sirius EcoSystem focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Sirius EcoSystem deployments remain first-class

That is a better fit for Sirius EcoSystem than either extreme:
